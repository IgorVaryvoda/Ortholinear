...
- The physical iPhone 15 Pro test passes in `build/Suggestions-physical-iphone-final.xcresult` (54.5 seconds), covering both languages, Space preserving a typo, replacement from inside a word, deletion, teaching, reopening, and forgetting. The first run exposed only a test expectation difference: an empty text view reports nil on the phone rather than an empty string. The assertion now accepts either representation.
- Physical extension diagnostics (Debug, 20 completed queries) report **16.0 ms warm p95**, **137 ms last cold model load**, and **31.1 MiB maximum sampled whole-extension footprint**. These exclude the 30 ms input debounce from lookup latency and are short-session samples, not sustained profiling or a Release latency guarantee. No typed text is logged. The earlier physical run measured 11.8 ms / 130.6 ms / 29.2 MiB, illustrating run-to-run variation.
- Signed Release build succeeds. After physical testing, version 0.4.0 (17) was restored on the paired iPhone (`build/suggestions-install-final.json`) and launched (`build/suggestions-release-launch.json`). The temporary test runner was removed. The initial locked-phone attempt is superseded by the successful unlocked run.
...
Before considering a neural model, expand independent spelling/preservation fixtures, evaluate acceptance/keystrokes saved on volunteered examples, and expand the physical-device measurements to sustained sessions, memory headroom, Release latency, and energy use. The existing 0.3.2 (16) App Review submission is separate from this development build.

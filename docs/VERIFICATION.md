...
Version 0.4.0 (18) adds Polish, German, French, and Spanish layouts, English Colemak/Colemak-DH/Dvorak/Workman variants, per-field-kind language memory, and the tap-only suggestions described in [suggestion verification and benchmarks](SUGGESTIONS-VERIFICATION.md). Build 18 replaces the submitted build 17 to put Languages and layouts first in Keyboard settings.
...
- All 13 in-app UI tests pass, including the new Languages and layouts test. Two tests that already failed before this release are fixed. `testPunctuationHoldAndCapsLock` dragged to the first key row, but the suggestion row moved the keys below the 38 pt alternatives strip; it now aims at the top of the keyboard. `testCustomizationPersistsAndMakesRoomForLetters` tapped toggles without scrolling them into view. With Languages first, the presets start below the fold, so settings tests scroll to each preset and the scroll helper tries the other direction halfway.
- Signed Release archive succeeds at `build/release-040-18/Ortholinear.xcarchive` with the App Group entitlement on both targets and `RequestsOpenAccess` false. The archived app installed and launched on the paired iPhone 15 Pro, which reports 0.4.0 (18). The first attempt failed while the phone was locked.

# Design review implementation — September 24, 2026 (unreleased)

Implements the recommendations from the September 24 product review: the hidden long-press popup fix, automatic capitals, double-space period, the punctuation row between words, digit flicks and an optional number row, two warm dictionaries, spell-checker suggestions for the other layouts, text replacements and contact names, in-keyboard tips and the test-drive checklist, wrong-layout recovery, glide typing, the Ukrainian ʼ apostrophe, and the settings seams (theme sheet title, English layout above the Russian question, the landscape hairline, the theme grid, the docked preview height).

- All 57 core tests pass (`swift test`). New coverage: capitals per field trait, double-space period and its timing, marks after an accepted suggestion's space, digit flicks and the number row with full hit coverage on every page, the ʼ apostrophe, replacement edits, wrong-layout recovery for the top 100 words each way, and glide decoding. On synthetic traces of the 100 most common words, glide top-1 is 96% in English and 99% in Ukrainian, and top-3 is 100% in both. A decode takes about 0.5 ms in release.
- All 16 in-app UI tests pass on a disposable iPhone 17 Pro simulator (iOS 26.5), including two new ones. `testSentenceCapitalsPunctuationRowAndDigitFlicks` covers a capital in an empty field, two Spaces typing “. ”, a suggestion followed by the row's period, and a flick on q typing 1. `testGlideTypingTypesAWholeWord` covers a w→t slide typing a whole word. Tests written before automatic capitals launch with `-no-auto-capitals`.
- All three installed-extension tests pass. An earlier run caught a crash: `requestSupplementaryLexicon` calls back on an XPC queue and tripped Swift 6's main-actor check. The list is now built off the main thread, then handed to the main actor.
- Simulator diagnostics (Debug, 20 queries) with both dictionaries loaded: 10.2 ms warm p95, 82 ms cold load, 55.8 MiB peak whole-process footprint. The single-dictionary simulator figure in 0.4.0 was 57.6 MiB. Measure on the oldest supported iPhone before release, since the practical limit is about 50–60 MiB.
- Not verified: the long-press popup is not asserted mid-hold, because XCUITest can't query during a press; the fix hides the suggestion row whenever the popup changes. Not checked on a device: the Liquid Glass container on iOS 26/27, and glide accuracy with real finger traces.
